...
POLICIES: list[dict] = [
    ...
    },
    {
        ...
        "description": "Blocks workforce user sign-ins classified as high risk by Identity Protection. Medium-risk sign-ins are challenged with MFA via CA103. This policy ensures high-risk sign-ins are outright denied, matching the admin (CA605) and guest (CA902) pattern.",
        "metadata": {"criticality": "Critical", "v2Status": "NEW", "persona": "All users", "j0eyvEquivalent": ""},
        "include": {"users": "all"},
        "exclude": {"groups": ["CA_ExcludedFromCA", "BG_BreakGlass", "CA_ServiceAccount"], "guestsAndExternals": True},
        "applications": {"include": "all"},
    ...
    {
        "id": "CA115",
        "displayName": "Step-up MFA for Sensitive Resources (Pilot)",
        "description": "Pilot pattern for authentication context: privileged actions tagged in target resources (SharePoint sites, Defender for Cloud Apps session controls, Purview labels, custom apps) with the authentication context value c1 are stepped up to phishing-resistant MFA before access is granted. Authentication context references (c1..c25) must be created in the Entra portal under Protection > Conditional Access > Authentication contexts before this policy binds them to a grant. Pair with downstream tagging in SPO/MDA/Purview to make the step-up meaningful.",
        "metadata": {"criticality": "Recommended", "v2Status": "NEW", "persona": "All users", "j0eyvEquivalent": ""},
        "include": {"users": "all"},
        "exclude": {"groups": ["CA_ExcludedFromCA", "BG_BreakGlass", "CA_ServiceAccount"], "guestsAndExternals": True},
        "applications": {"include": "none"},
        "conditions": {"authenticationContextClassReferences": ["c1"], "clientAppTypes": ["all"]},
        "grant": {"operator": "OR", "authenticationStrength": "phishingResistantMfa"},
    },
    {
        "id": "CA116",
        "displayName": "Require Compliant Network for SaaS (Pilot)",
        "description": "Pilot control requiring sign-ins to originate from a Microsoft Entra Internet Access / Private Access compliant network (Global Secure Access). Skip this policy if your tenant does not have Global Secure Access or the Entra Suite licensed and deployed to client devices. Recommended to pilot in report-only against a small population (and validate GSA client health) before broadening. Blocks any sign-in NOT coming through the compliant network tunnel.",
        "metadata": {"criticality": "Optional", "v2Status": "NEW", "persona": "All users", "j0eyvEquivalent": ""},
        "include": {"users": "all"},
        "exclude": {"groups": ["CA_ExcludedFromCA", "BG_BreakGlass", "CA_ServiceAccount"], "guestsAndExternals": True},
        "applications": {"include": "all"},
        "conditions": {
            "locations": {"include": ["All"], "exclude": ["AllCompliantNetwork"]},
            "clientAppTypes": ["all"],
        },
        "grant": {"operator": "OR", "builtInControls": ["block"]},
    },
    {
        "id": "CA117",
        "displayName": "Block High Insider Risk",
        "description": "Consumes the Microsoft Purview Insider Risk Management adaptive protection signal exposed to Conditional Access. Blocks sign-ins from workforce users currently elevated to the high adaptive insider risk tier (e.g., users exfiltrating large volumes after a triggering HR event). Requires Microsoft Purview Insider Risk Management configured with adaptive protection enabled, plus Entra ID P2 (and the Purview licensing prerequisites) on the in-scope users. Pair with CA103b (high sign-in risk) and CA102 (high user risk) for a layered identity + insider risk response.",
        "metadata": {"criticality": "Recommended", "v2Status": "NEW", "persona": "All users", "j0eyvEquivalent": ""},
        "include": {"users": "all"},
        "exclude": {"groups": ["CA_ExcludedFromCA", "BG_BreakGlass", "CA_ServiceAccount"], "guestsAndExternals": True},
        "applications": {"include": "all"},
        "conditions": {"insiderRiskLevels": ["elevated"], "clientAppTypes": ["all"]},
        "grant": {"operator": "OR", "builtInControls": ["block"]},
    },
